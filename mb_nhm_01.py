...
# Looks at all loose files in 'gexf/directed/' with a '.gexf' extension (directed-gexf files output by 'mb_network_multi')
...
logPath = os.path.join("logs", 'mb_nhm_' + versionNumber + '_log.txt')
runLog = open(logPath, 'a')

# open file for plot - file output
plotPath = os.path.join("data", 'mb_nhm_plot.txt')
...
resultsPath = os.path.join("results", 'mb_nhm_' + versionNumber + '.txt')
...
for index in range(len(fileNames)):
	...
	for node in diEnGraph.nodes():
		...
		for line in genreInput:
			vMbid, vArtistStart, vCountry = line.split(",")
		...
		if nodeInDeg > 0:
			...
			for edge in inEdges:
				...
				for line in sourceGenreFile:
					mbid, artistStart, country = line.split(",")
					MB = str(mbid).replace("\n","") 
